...
$fpdf->SetFont('Times','',11);
	...
	$fpdf->SetFont('Times','',8);
	$fpdf->Text(8.1,2.7,strtoupper($lacase['Lacase']['la_case_number']));
	$fpdf->Text(5.8,3.3,"Proceedings under Section 9(3A) of West Bengal Ammendment Act 1999");
...
#============ Right Hand Side =============
	...
	$fpdf->Cell(3.4,1,'',1,0);
	//$fpdf->Cell(6.8,1,'',1,0);
	$fpdf->MultiCell(6.5,0.5,'The date of possession to prior tentative metirial date('.date('d.m.Y',strtotime($lacase['Lacase']['possession_date'])).' to '.date('d.m.Y',strtotime($lacase['Lacase']['publication_date'])).')',1,1);
...
/*        $diff = abs(strtotime($end_date) - strtotime($start_date));
	...
	$fpdf->ln(3);
	$periodforAdditionalCompensation1 = Get_Date_Difference($lacase['Lacase']['possession_date'] ,$lacase['Lacase']['period_of_interest_from']);
	...
	$fpdf->MultiCell(13,0.5,'(1) The date of possession to prior date of 80% payment @ 12% p.a on Rs. '.number_format($total_old_value,2).' ie.'.date('d.m.Y',strtotime($lacase['Lacase']['possession_date'])).' to '.date('d.m.Y',strtotime($lacase['Lacase']['publication_date'])).' ('.$period.') =                        Rs. '.number_format($comp1,2),0,1);
	
	$periodforAdditionalCompensation2 = Get_Date_Difference($lacase['Lacase']['period_of_interest_from'] ,$lacase['Lacase']['publication_date']);
	...
	$fpdf->MultiCell(13,0.5,'(2) From the date of 80% payment to prior tentative metirial date @ 12% p.a on Rs. '.number_format($total_eighty_paid_old,2).' ie.'.date('d.m.Y',strtotime($lacase['Lacase']['period_of_interest_from'])).' to '.date('d.m.Y',strtotime($lacase['Lacase']['publication_date'])).' ('.$period.') =                       Rs. '.number_format($comp2,2),0,1);
...
$fpdf->Text(0.7,26.5,'Asstt. L.A.O, Purba Madinipur');
	$fpdf->Text(5,26.5,'Addl. LAO, Purba Madinipur');
	$fpdf->Text(9,26.5,'Spl. LAO, Purba Madinipur');
	$fpdf->Text(13,26.5,'A.D.M.(L.A.), Purba Madinipur');
	$fpdf->Text(17,26.5,'Collector, Purba Madinipur');

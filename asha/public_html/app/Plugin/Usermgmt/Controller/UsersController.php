...
class UsersController extends UserMgmtAppController {
	...
	public function admin_panel() {
		# This function will show options for ADMIN to EXECUTE
	}
	...
	public function customer_register() {
		...
		if($this->Session->read('UserAuth.User.id'))
			...
			$this->redirect(array('plugin'=>false,'controller'=>'billing_details','action'=>'add'));
		...
		if ($userId) {
			...
			//	if ($this->User->RegisterValidate()) {
					...
					if (isset($this->request->data['User']['active']) && $this->request->data['User']['active']) {
						...
						$this->Session->setFlash(__('You have been registered. Please login to access your account.'));
						$this->redirect('/login');
					}
		...
		if ($this->request -> isPost()) {
			...
			if($this->User->LoginValidate()) {
				...
				if ($user['User']['password'] === $hashed) {
					...
					// $this->Session->delete('Usermgmt.OriginAfterLogin');
					// $redirect = (!empty($OriginAfterLogin)) ? $OriginAfterLogin : loginRedirectUrl;
		//				die('customer login - before billing details');

					$redirect = '/BillingDetails/add';
					$this->redirect($redirect);
				} else {
					$this->Session->setFlash(__('Incorrect Email/Username or Password'));
					return;
		...
		if (siteRegistration) {
			...
			if ($this->request -> isPost()) {
				...
				if ($this->User->RegisterValidate()) {
					...
					*/			
					//$user = $this->User->query("SELECT DISTINCT * FROM users as User where id = $userId");			
					//$user = $user ['0'];
					//debug($user);
